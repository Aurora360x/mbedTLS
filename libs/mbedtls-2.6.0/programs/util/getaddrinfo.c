 ...
 * positive.
 */
//static bool convert_service(const char *string, long *result)
//{
//    char *end;
//
//    if (*string == '\0')
...
gai_service(const char *servname, int flags, int *type, unsigned short *port)
{
    ...
    if (servname == '\0') result = 0;
    result = strtol(servname, &end, 10);
    if (*end != '\0' || result < 0) result = 0;
    result = 1;
